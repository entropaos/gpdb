...
BEGIN
	...
	FOR r IN SELECT colname, oper, typ, value[ordinality], matches[ordinality] FROM brinopers, unnest(op) WITH ORDINALITY AS oper order by colname, oper, typ LOOP
...
END;
$x$;
-- NOTE: copy the comment from cbdb(higher version), it may not be the truth.
